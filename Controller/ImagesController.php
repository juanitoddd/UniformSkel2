...
class ImagesController extends AppController {
	...
	function preview(){
        ...
        $extention = explode('.', $this->params['filename']);
        $ext = $extention[sizeof($extention)-1];

        $this->RequestHandler->respondAs('image/'.$ext);
		...
		$this->data = array('path' => $this->params['path'], 'filename' => $this->params['filename'], 'extension' => $ext);
	}
    ...
    function upload($target) {
        ...
        if (!$chunks || $chunk == $chunks - 1) {
            ...
            $filePath = preg_replace('/files/', '', $filePath);
            die('{"result" : true, "error" : false, "id" : "id", "file" : "'.$filePath.'"}');
        }
